...
class imdb(object):
    ...
    def get_multiclass_train(self, split, group, num_folds=4):
        ...
        sample_img_ids_1 = np.random.choice(len(cat_list), 1+5, replace=False)
        query_img = cat_list[sample_img_ids_1[0]]
        support_img = cat_list[sample_img_ids_1[1]]
        # support_img = [cat_list[sample_img_ids_1[i]] for i in range(1,6)]
    ...
    def get_nway_nshot_train(self, split, group, num_folds=4, nway=1, nshot=1, nquery=1):
        cats = self.get_cats('train', group, num_folds)
        rand_cat = np.random.choice(cats, 1, replace=False)[0]
        cat_list = self.multiclass_grouped_imgs[rand_cat]
        sample_img_ids_1 = np.random.choice(len(cat_list), nshot+nquery, replace=False)
        # query_img = cat_list[sample_img_ids_1[0]]
        # support_img = cat_list[sample_img_ids_1[1]]
        query_img = [cat_list[sample_img_ids_1[i]] for i in range(nquery)]
        support_img = [cat_list[sample_img_ids_1[i]] for i in range(nquery,nquery+nshot)]
        return query_img, support_img, rand_cat


    def get_2way_nshot_train(self, split, group, num_folds=4, nway=1, nshot=1, nquery=1):
        cats = self.get_cats('train', group, num_folds)
        rand_cat = np.random.choice(cats, nway, replace=False)
        support_img_list = []
        for categr in (rand_cat):
            cat_list = self.multiclass_grouped_imgs[categr]
            sample_img_ids_1 = np.random.choice(len(cat_list), nshot+nquery, replace=False)
            query_img = [cat_list[sample_img_ids_1[i]] for i in range(nquery)]
            support_img = [cat_list[sample_img_ids_1[i]] for i in range(nquery, nshot+nquery)]
            support_img_list.append(support_img)
        return query_img, support_img_list, rand_cat

    def get_2way_nshot_test(self, split, group, num_folds=4, nway=1, nshot=1, nquery=1):
        cats = self.get_cats('val', group, num_folds)
        rand_cat = np.random.choice(cats, 2, replace=False)
        support_img_list = []
        for categr in (rand_cat):
