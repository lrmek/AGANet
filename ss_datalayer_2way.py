    ...
    def next_pair(self):
        with self.lock:
            ...
            if self.params['output_type'] == 'single_image':
                ...
                player_1 = util.VideoPlayer(imgset_1, base_trans, self.params['image_frame_trans'])
                player_2 = util.VideoPlayer(imgset_2, base_trans, self.params['image_frame_trans'])

                set_indices_1 = range(second_index_1) + range(second_index_1 + 1, player_1.length)
                self.rand_gen.shuffle(set_indices_1)
                first_index_1 = set_indices_1[:self.params['k_shot']]
                set_indices_2 = range(second_index_2) + range(second_index_2 + 1, player_2.length)
...
class PairLoaderProcess(Process):
    ...
    def __get_deploy_info(self, player, index):  # index=50
        if index is None:
    ...
    def load_frame(self, player1, player2, first_index_1, first_index_2, second_index):
        ...
        item = dict(first_img=[images1, images2], image1_path=[image_path_1, image_path_2])

        if second_index is not None:
            frame2_dict = player1.get_frame(second_index)
            image_q, label_q, shape = self.__prepross(frame2_dict, self.second_shape)
        ...
        if self.deploy_mode:
            ...
            deploy_info = {}
            if second_index is not None:
                second_semantic_labels, second_mask_orig, second_img_orig = self.__get_deploy_info(player1,
                                                                                                   second_index)
        ...
        for i in range(len(self.first_label_params)):
            ...
            for label2 in labels2:
                ...
                nlabel2 = (nlabel2 - self.first_label_mean) * self.first_label_scale
...
class SSDatalayer():
    def __init__(self, test_group, k_shot=5):
        ...
        self.param_str = {
            ...
            'has_cont': False,
            'deploy_mode': True,
            'pascal_path': '/home/liruimin/SG-One-master/data/VOCdevkit2012/VOC2012'
        }
    ...
    def setup(self):
        ...
        assert (set(self.top_names) <= set(self.all_top_names)), str(self.top_names) + ' is not subset of ' + str(
            self.all_top_names)
